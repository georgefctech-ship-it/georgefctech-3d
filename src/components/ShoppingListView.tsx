...
export default function ShoppingListView({
  ...
  // Excel Link-Friendly Format CSV Generation
  const generateCSV = () => {
    ...
    const headers = [
      ...
      'Status de Aquisição',
      'Link para Compra Direta',
      'Notas / Observações'
    ];

    const rows = shopping.map(item => {
      return [
        item.id,
        item.materialName,
        item.category,
        ...
        (item.qtyNeeded * item.estUnitCost).toFixed(2),
        item.checked ? 'Comprado / Recebido' : 'Pendente / Planejado',
        item.purchaseLink || '',
        item.notes || ''
      ];
    });
    ...
    const csvContent = "\uFEFF" + [
      headers.join(';'),
      ...rows.map(r => r.map(fv => `"${fv.replace(/"/g, '""')}"`).join(';'))
    ].join('\r\n');
    ...
    downloadAnchor.setAttribute('download', `Pedido_de_Compras_GeorgeFctech_${new Date().toISOString().split('T')[0]}.csv`);
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200 select-none">
                  <th className="py-3 px-5 text-[10px] font-bold uppercase tracking-wider text-slate-500 font-mono w-12 text-center">Status</th>
                  ...
                  <th className="py-3 px-4 text-[10px] font-bold uppercase tracking-wider text-slate-500 font-mono text-center">Ações</th>
                </tr>
              </thead>
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-start gap-3">
                      <button
                        onClick={() => onToggleShoppingItemChecked(item.id)}
                        className={`mt-1.5 h-6 w-6 rounded border flex items-center justify-center transition cursor-pointer ${
                          item.checked 
                            ? 'bg-emerald-600 border-emerald-600 text-white' 
                            : 'border-slate-300 hover:border-indigo-500 bg-white'
                        }`}
